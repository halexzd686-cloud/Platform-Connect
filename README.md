...
Platform Connect 是一个可安装的 Agent Skill：直接读取用户粘贴的文章、TXT、Markdown、Word、PDF、HTML 或文章链接，建立共享事实基线，再生成平台原生文案、经授权的视觉资产和结果型离线看板。默认使用紧凑审阅，也支持逐步审批和明确预授权的自动执行。
...
平台未指定时，第一次回复只做决策所需的推荐，不提前为全部候选平台写完整文案：

```text
用户直接发送文章
  → Agent 推荐 2–3 个平台与初步视觉方向
  → 用户一次确定平台、是否配图、审阅或直接完成
  → Agent 继续生成审阅包或直接交付
```
...
- `compact`：默认；平台未指定时先合并询问平台选择、配图意图和完成方式，随后用一次联合确认解决文案、视觉方向和资产清单。
...
```text
使用 $platform-connect 直接读取这篇文章。
如果我没有指定平台，推荐 2–3 个国内外平台和配图方向，再让我一次选择。
```
...
python skills/platform-connect/scripts/prepare_workspace.py demo `
  ...
  --source-type file `
  --source-ref article.pdf `
  --source-media-type application/pdf `
  --target-language en `
  --market global
...
- 原文始终是事实来源。
- 用户可以直接发送常见文档、正文或文章链接，不需要先编写复杂提示词。
- 不默认选择所有平台。
- 未指定平台时默认推荐两个、最多三个，并优先提供有意义的国内外组合。
- 平台适配改变开场、节奏、结构与 CTA，不改变事实。
- 海外平台明确记录目标语言和市场。
- 默认使用一次联合确认；只有高风险歧义或用户要求时才拆分审批。
- 图片生成必须有明确、联合、配置或预授权来源，不能由 Agent 推断。
- 自定义提示词与推荐视觉方向地位相同。
- 静态看板优先展示最终文案与真实图片，过程记录默认折叠；它可通过 `file://` 直接打开，无 CDN、无远程请求、无服务器。
